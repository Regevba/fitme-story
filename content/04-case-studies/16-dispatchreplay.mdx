...
date: '2026-04-21'
tldr: "Static blueprint diagrams tell you what a system is, not whether it's running. DispatchReplay plays a recorded trace of a real feature flowing through six framework floors, beat by beat. Two traces — Sprint I (mechanical work) and the meta-recursive fitme-story build itself."
key_numbers:
  - label: 'Live traces'
    value: '2 (Sprint I + fitme-story meta)'
    tier: T1
  - label: 'fitme-story trace dispatches (vs estimated 111)'
    value: '23'
    tier: T1
  - label: 'Beats per trace / floors per beat'
    value: '7 / 6'
    tier: T1
honest_disclosures:
  - 'Replay, not simulator — the trace is recorded and deterministic. Live re-execution would require running the framework against shared/JSON configs.'
  - 'The fitme-story meta-trace shows the framework building this site — recursion is intentional, not staged for the demo.'
  - 'Three interactivity tiers debated (replay / interactive / live simulator); replay won on cost-to-value — the other two are not built.'
visual_aid:
  component: DispatchReplay
  data:
    traceId: 'sprint-i'
    allowTraceSwitch: true
category: dev-deepdive
---


<DevDive>
React + Framer Motion · IntersectionObserver scroll-sync · meta-recursive trace design · Next.js 16 client components in MDX. PM/HR readers: this one is about how the animated demo on `/framework/dispatch` was designed and built — skip unless you want the motion-grammar rationale.
</DevDive>
...
<DispatchReplay traceId="sprint-i" allowTraceSwitch={true} />

*Above: the component this case study is about. Click play to watch a 10-commit Sprint-I run flow through six framework floors. Switch to the `fitme-story` trace to see the meta-trace — this site being built.*
