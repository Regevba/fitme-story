...
visual_aid:
  ...
  data:
    value: '1 of 11'
    label: 'deferred P2 closed — the only one whose blocker (a Tag<muted> variant) was tractable without designer input or visual-regression infra'
    accentVar: 'var(--color-brand-indigo)'
    context: 'Lifts the parent feature P2 closure rate 4/16 → 5/16 (31%). The other 10 stay deferred with their per-item reasons — judgement calls about hero gradients, heading scales, and stat-component regression do not belong inside a mechanical cleanup pass.'
honest_disclosures:
  - >-
