...
visual_aid:
  ...
  data:
    before:
      label: 'v1 (audit UI-016)'
      value: '0 events fired'
      subtitle: >-
        Substrate shipped, tested, reviewed, and merged — but the priming view
        was never instantiated and NotificationService was never called from app
        lifecycle. A complete feature nobody could see. No gate caught the dead
        ship.
      accentVar: 'var(--color-neutral-500)'
    after:
      label: 'v2 (platform layer)'
      value: '3 reachability tests'
      subtitle: >-
        Rebuilt as a platform layer (gateway + registry + deep-link router +
        observers). 3 XCTest reachability cases (<10ms total) now assert every
        wire is reachable from a real navigation path — dead-ship can't recur.
      accentVar: 'var(--skill-qa)'
    delta: 'dead-shipped substrate → reachability-gated platform'
kill_criteria:
  - >-
