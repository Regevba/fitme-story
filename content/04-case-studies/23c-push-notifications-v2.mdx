...
key_numbers:
  ...
  - label: "Tasks done"
    value: "16 / 16"
    tier: T1
    note: "7 P-core (heavyweight) + 9 E-core (lightweight); critical path T1→T3→T6→T14→T15→T16"
  - label: "Tests pass"
    value: "36 / 36"
    tier: T1
    note: "30 unit + 3 reachability gate + 2 analytics-firing (MockAnalyticsAdapter) + 1 misc"
  - label: "Reachability gate cases"
    value: "3"
    tier: T1
    note: "FirstWorkoutTrigger fires once + every URL routes + notification-source end-to-end. The 3rd case is the test that, had it existed for smart-reminders, would have caught its own .fitMeReminderTapped no-consumer gap."
  - label: "CI checks on PR"
    value: "8 / 8"
    tier: T1
    note: "Build-and-Test 14m53s + integrity 24s + pm-framework/pr-integrity 'PASS — no new findings vs main' + 5 others"
  - label: "Net-new LOC"
    value: "~1,850"
    tier: T2
    note: "1,174 platform code + 567 test code + ~110 wiring/edits across 13 new + 7 edited + 5 HISTORICAL + 1 deleted file"
  - label: "Bugs caught at build verification"
    value: "1"
    tier: T1
    note: "ReadinessResult.score → .overallScore typo; one-line fix; build re-ran clean. The build-verify gate works as intended."
  - label: "Figma frames auto-built"
    value: "4"
    tier: T1
    note: "New page 936:2 'Push Notifications v2'; surfaces 937:6 (priming sheet) + 937:46 (Settings row 3 states) + 938:2 (denial banner) + 938:50 (readinessAlert high+low). 2 iterations (sizing-mode reset bug on first pass)."
---
