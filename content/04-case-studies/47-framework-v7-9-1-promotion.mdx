...
visual_aid:
  ...
  data:
    ...
    context: >-
      Opened at v7.9 Phase E exit (2026-06-04) and closed the same day. Every
      ship was an observability surface, doc update, reusable substrate, or
      warn-only CI workflow — 5 of the 8 closed documented silent-pass classes.
      CI workflows grew 8 → 14; FT2 dev-env open R-items 7 → 0. 5 cascading
      rebases, zero manual conflict resolution, zero rollbacks.
category: framework
---
...
### Adjacent v7.9.1-era hardening ships (2026-06-05 → 06-06)

Two further v7.9.1-tagged framework-hardening features shipped in the days immediately after this single-day build window; they are recorded here for completeness (each carries its own FT2 source case study):

| Feature | What | PRs |
|---|---|---|
| `w9-drift-triggered-auto-isolation` | Auto-isolate into a git worktree when W9 branch-drift is detected mid-session | #646 / #648 / #649 |
| `framework-w30-w31-w32-durable-fixes` | Durable fixes for the W30 YAML-parity parser bare-integer drop, the W31 workflow-delivery anomaly, and W32 `close-feature.py --force-incomplete` | durable-fix batch |
...
1. **F16 advisory→enforced flip** — **enforced 2026-06-17 (PR #764)**, one day early. Not a single-line code flag like v7.9: the `try-repo-harness` CI job was added to main's required status checks via `gh api` branch protection (an integration-test gate at the CI layer). K2 false-positive rate 0% over 60 CI runs / 13 days.
