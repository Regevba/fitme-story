...
> 13 timeline studies + 8 deep-dives + the v7.6 mechanical-enforcement case study. One complexity model. A linear progression that traces how an AI-assisted PM framework evolved from a 6.5-hour pilot through hardware-aware dispatch (v7.0), the 72h integrity cycle (v7.1), the Data Integrity Framework (v7.5), and Mechanical Enforcement (v7.6) — with honest measurements at every step, and explicit outlier-flagging where they apply.

## Context

These case studies document the evolution of an AI-assisted PM framework across features built between v2.0 and v7.6. Unlike typical "we shipped fast" narratives, every claim here is backed by a normalized complexity metric (<Term slug="complexity-units">Complexity Units</Term>, or CU) that accounts for task count, work type, and difficulty factors like auth integration, UI complexity, and architectural novelty. The result: apples-to-apples velocity comparisons across features of wildly different scope.

**Read them in order.** The numbering follows the framework's version timeline — v2.0 through v7.6. Each study builds on the previous, and the progression from "manual and slow" to "parallel, measured, hardware-aware, and mechanically enforced" is the point. The v7.5 + v7.6 case studies (slot 21 here, plus the upstream long-form versions in [FitTracker2 docs](https://github.com/Regevba/FitTracker2/tree/main/docs/case-studies)) are the framework's response to the [2026-04-21 Google Gemini 2.5 Pro independent audit](https://fitme-story.vercel.app/trust/audits/2026-04-21-gemini); the v7.6 case study is explicitly **outlier-flagged** (single-session, dogfooded, retroactive v6.0).
...
> Slots 14–20 are deep-dives that branch from the main timeline (framework story site, SSR regression, dispatch-replay, Lego-PM-flow, dual-sync race, stacked-PR misfire, XCTWaiter abort/retry). Slot 21 is the v7.6 audit-response case study.
