...
> 13 timeline studies + 8 deep-dives + the v7.6 mechanical-enforcement case study + the v7.7 validity-closure case study. One complexity model. A linear progression that traces how an AI-assisted PM framework evolved from a 6.5-hour pilot through hardware-aware dispatch (v7.0), the 72h integrity cycle (v7.1), the Data Integrity Framework (v7.5), Mechanical Enforcement (v7.6), and Validity Closure (v7.7) — with honest measurements at every step, and explicit outlier-flagging where they apply.

## Context

These case studies document the evolution of an AI-assisted PM framework across features built between v2.0 and v7.7. Unlike typical "we shipped fast" narratives, every claim here is backed by a normalized complexity metric (<Term slug="complexity-units">Complexity Units</Term>, or CU) that accounts for task count, work type, and difficulty factors like auth integration, UI complexity, and architectural novelty. The result: apples-to-apples velocity comparisons across features of wildly different scope.

**Read them in order.** The numbering follows the framework's version timeline — v2.0 through v7.7. Each study builds on the previous, and the progression from "manual and slow" to "parallel, measured, hardware-aware, mechanically enforced, and validity-closed" is the point. The v7.5 + v7.6 + v7.7 case studies (slots 21–22 here, plus the upstream long-form versions in [FitTracker2 docs](https://github.com/Regevba/FitTracker2/tree/main/docs/case-studies)) are the framework's response to the [2026-04-21 Google Gemini 2.5 Pro independent audit](https://fitme-story.vercel.app/trust/audits/2026-04-21-gemini); the v7.6 case study is explicitly **outlier-flagged** (single-session, dogfooded, retroactive v6.0).
...
| 12 | [<Term slug="hadf">V7.0 HADF</Term>](12-hadf.md) | v7.0 | Hardware | 17 chip profiles + 7 cloud signatures in 120 min |
...
| 21 | [Mechanical Enforcement (v7.6)](21-mechanical-enforcement-v7-6.md) | v7.5 → v7.6 | Audit response | 7 Class B → A promotions + 5 explicit unclosable Class B gaps; outlier-flagged single-session work |
| 22 | [Validity Closure (v7.7)](22-validity-closure-v7-7.md) | v7.7 | Validity closure | 5 new check codes (4 gating + 1 advisory permanent — kill-2 fired honestly), framework-health dashboard live, cache_hits writer-path closed (5 → 4 unclosable), linkage 95.5% → 100%, doc-debt 4–61% → 95.7–100%, single-session ship |

> Slots 14–20 are deep-dives that branch from the main timeline (framework story site, SSR regression, dispatch-replay, Lego-PM-flow, dual-sync race, stacked-PR misfire, XCTWaiter abort/retry). Slot 21 is the v7.6 audit-response case study; slot 22 is the v7.7 validity-closure case study.
...
**If you have 45 minutes:** Add slot 21 (Mechanical Enforcement v7.6) and the [trust page](https://fitme-story.vercel.app/trust/audits/2026-04-21-gemini) to that sequence. The v7.6 case study is the framework's response to the Gemini independent audit and the explicit closure of v7.5 → v7.6.

**If you are a developer:** read the [Developer guide (v1.0 → v7.6)](https://fitme-story.vercel.app/framework/dev-guide) instead — 745 lines covering the wiring (4 enforcement layers, `state.json` schema, phase lifecycle, dispatch model, cache architecture, measurement protocol, 12 integrity check codes, 3 operational walkthroughs).
