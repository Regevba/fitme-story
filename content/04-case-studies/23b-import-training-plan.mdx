---
title: >-
  Import Training Plan — Resume from Audit-Flagged Partial Ship to Full Phase 1
  Ship in 14 Hours
slug: import-training-plan
upstream_path: docs/case-studies/import-training-plan-case-study.md
tier: light
tldr: >-
  Resumed an audit-flagged partial-ship feature: rolled back to research
  mid-flight after discovering the original PRD claimed an impossible
  persistence target, rewrote the PRD against the actual write path, shipped
  Phase 1 (persist + activate + GDPR + 9 analytics events). Same session
  produced a v4.X skill-layer upgrade as a meta-byproduct: 4 new mechanical
  gates + auto Figma build. 4 PRs across 2 repos, 18/18 tasks, 33 new tests, 4
  Figma frames auto-built (first v4.X production run), 0 P0 in ui-audit.
timeline_position:
  version: '7.8'
  order: 37
persona_emphasis:
...
key_numbers:
  - label: PRs landed
    ...
    tier: T1
    note: >-
      FT2 #234 (feature) + #235 (skill upgrade) + #236 (backlog cleanup) +
      fitme-story #48 (skill reflection)
  - label: Tasks done
    value: 18 / 18
    tier: T1
  ...
  - label: Tests added
    ...
    tier: T1
    note: >-
      33 new tests across persistence + routing + orchestrator + GDPR + 11
      analytics; 49/49 import-related suites pass
  ...
  - label: Figma frames auto-built
    ...
    tier: T1
    note: >-
      v4.X /design build's first production run; page 916:2 in design system
      library
visual_aid:
  component: BeforeAfter
  data:
    before:
      label: 'Pre-resume (partial ship)'
      value: '0 entry points'
      subtitle: >-
        Audit UI-015 open. No persistence path (TrainingProgramData is a struct
        of static lets; confirmImport() was a no-op). Views existed but were
        never wired into navigation.
      accentVar: 'var(--color-neutral-500)'
    after:
      label: 'Post-resume (Phase 1 ship)'
      value: '2 entry points'
      subtitle: >-
        Audit UI-015 closed. Persists to EncryptedDataStore.importedTrainingPlans
        (encrypted, GDPR Art-17/20), reachable from Settings list + Training
        toolbar. 9 analytics events wired through the consent gate.
      accentVar: 'var(--skill-dev)'
    delta: 'partial ship → full Phase 1 in one ~14h resume'
kill_criteria:
  - >-
    Encrypted persistence layer regression (any read/write failure on
    importedTrainingPlans)
  - >-
    Active-plan switching introduces a measurable cohort drop in workout
    completion (>5% week-1 vs prior week)
  - GDPR Art-17 export/delete fails any audit dry-run
kill_criterion_fired: false
honest_disclosures:
  - >-
    The original PRD persistence claim was structurally impossible. v1 PRD said
    "ImportOrchestrator writes to TrainingProgramData" but TrainingProgramData
    is a struct with only static let fields. Audit caught the partial ship;
    resume caught the architectural gap. Cost: ~1 day rollback that wouldn't
    have been needed if v1 PRD had referenced file:line touch points.
  - >-
    User-ordered pre-Phase-4 audit caught 4 P0 spec errors before any code was
    written: AppRadius.pill, AppMotion.standardEase, SettingsActionLabel custom
    badge slot, toast/snackbar component — none existed in the codebase. Audit
    cost ~20 min; Phase 4 rework would have cost ~2-4h. This pattern is now
    mechanical via /ux preflight + /design preflight (PR #235).
  - >-
    Phase 2 sync is deferred. CloudKit and Supabase per-record sync for imported
    plans is out of Phase 1 scope. The needsSync field exists on the model from
    day one so Phase 2 can opt records in without a schema migration.
category: product
---
