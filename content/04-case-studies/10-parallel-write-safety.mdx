...
upstream_path: docs/case-studies/parallel-write-safety-v5.2-case-study.md
tier: light
timeline_position:
  version: '5.2'
  order: 10
...
date: '2026-04-15'
tldr: 'v5.1 stress test had 0 file conflicts across 15 same-file edits — by luck, not by design. v5.2 Parallel Write Safety (snapshot/rollback + region-based mirror pattern + 3-tier region detection that warms toward Tier 1 over time) makes the zero-conflict guarantee structural.'
key_numbers:
  - label: 'Same-file edits without conflict'
    value: '15 across 3 files'
    tier: T1
  - label: 'Wall time (design → ship)'
    value: '20 minutes'
    tier: T2
...
visual_aid:
  ...
  data:
    ...
    beforeValue: '0 by luck'
    beforeSubtitle: '15 same-file edits across 3 files happened to land on non-overlapping regions'
    afterLabel: 'v5.2 (write safety)'
    afterValue: '0 by design'
    afterSubtitle: 'snapshot/rollback + region mirror + 3-tier region detection (Tier 1 instant after first run)'
    delta: 'probabilistic → structural'
---
...
<Term slug="parallel-write-safety">Parallel Write Safety</Term> (v5.2, Sub-Project B) was designed to make the zero-conflict guarantee structural rather than lucky.
...
## How It Complements <Term slug="dispatch-intelligence">Dispatch Intelligence</Term>
