...
visual_aid:
  ...
  data:
    value: '2 of 6'
    label: 'features from the original roundup that stay roundup-only — too thin for a dedicated case study (3 dense siblings split to slots 24a/b/c, 1 already had its own)'
    context: 'The honest disposition: 2 pre-rule features (development-dashboard, ai-cohort-intelligence) have only state.json + a PR diff, so a dedicated study would mean fabrication or file-tour redundancy. They stay here as historical record.'
honest_disclosures:
