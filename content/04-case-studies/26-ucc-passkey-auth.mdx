...
visual_aid:
  ...
  data:
    value: '0'
    label: 'shared phishable passwords on /control-room/* — replaced by per-operator WebAuthn passkeys'
    context: >-
      iOS users shipped passkeys on 2026-05-01; the operator dashboard was still
      gated by one shared HTTP basic-auth credential. This feature closes that
      asymmetry — per-operator identity, per-device credentials, server-side
      session allowlist (instant revoke). Target: 0 unauthenticated dashboard
      reads/day post-cutover.
