...
visual_aid:
  ...
  data:
    value: '4 / 4'
    label: 'deferred security gaps from the parent PRD closed before the v7.9 freeze — allowlist, hybrid Redis lockout, token-issuance audit, operator unlock CLI'
    accentVar: 'var(--skill-qa)'
    context: 'T+7d kill-criteria verdict (2026-05-27): PROMOTE. Zero false-positive lockouts, zero unauthorized registrations, 7/7 token-issuance audit coverage — at +~2ms auth-path latency from one extra Redis GET.'
kill_criteria:
