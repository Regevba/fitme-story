...
tldr: >-
  ...
  premise. **Status: Sub-exp 1 PASS + Sub-exp 2 PASS (closed 2026-06-02);
  Sub-exp 3 pending (gated on AWS Bedrock access). Synthesis disposition held
...
**DRAFT — Sub-exp 1 + 2 closed, Sub-exp 3 pending.** This case study is a Block C pre-work skeleton authored 2026-05-27 (Phase E Day 6) before sub-exp data was collected. Verdict cells populate as each sub-exp closes: **Sub-exp 1 PASS** (cloud generalization) and **Sub-exp 2 PASS** (cloud-vs-local separability, results below, closed 2026-06-02) are in; **Sub-exp 3** (the routing falsification test) is gated on AWS Bedrock access. Anchor-drift analysis (§4) populates after Sub-exp 3 closure. Overall HADF claim status (§5) populates after all three sub-exp verdicts + anchor-drift are in.
