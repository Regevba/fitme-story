---
title: >-
  Unified Control Center — Migrating an Operator Dashboard Across Two Repos in
  11 Days
slug: unified-control-center
upstream_path: docs/case-studies/unified-control-center-case-study.md
tier: light
tldr: >-
  Retired the legacy Astro operator dashboard, migrated it inside the public
  showcase as a basic-auth-gated /control-room route, instrumented all 8 GA4
  events, killed the standalone Vercel project. 11 days, 42/44 tasks done + 1
  deferred, 20 PRs across 2 repos, 0 showcase regressions. Pre-state TTC
  baseline unmeasurable; post-state establishes the production baseline.
timeline_position:
  version: '7.8'
  order: 36
persona_emphasis:
...
key_numbers:
  ...
  - label: Showcase regressions
    ...
    tier: T2
    note: >-
      declared from build success across all PRs; no formal regression test
      wired
visual_aid:
  component: HeroMetric
  data:
    value: '0'
    label: 'showcase regressions across a 20-PR, 2-repo, 11-day dashboard migration'
    context: >-
      Retired the legacy standalone Astro dashboard, migrated it inside the
      public showcase as a basic-auth-gated /control-room route, instrumented
      all 8 GA4 events, and killed the standalone Vercel project — 42/44 tasks
      done — without a single showcase regression.
    accentVar: 'var(--skill-release)'
kill_criteria:
  - >-
    Auth (proxy.ts basic-auth) blocks legitimate operator access — repeated 401s
    on valid credentials
  - >-
    GA4 instrumentation regressions block ≥3 of the 8 events from firing in
    production
  - TTC degradation > 50% vs measured baseline once GA4 produces 7 days of data
kill_criterion_fired: false
honest_disclosures:
  - >-
    Pre-state TTC baseline is unmeasurable. Legacy Astro dashboard GA4
    instrumentation had effectively stopped firing by 2026-05-06; 2 events from
    a single user across 7 days is not a usable sample. T2.5 deferred (status:
    deferred); post-launch GA4 establishes the production baseline.
  - >-
    CI side-fix landed during the window. Multi-week parallel-clone simulator
    hang env-flake (~24 failed runs over 6 days, forcing admin-merges) was
    diagnosed and fixed mid-UCC via FT2 PR #225. Two-layer root cause:
    FitTracker.xcscheme parallelizable=YES + AuthPolishV2UITests
    zombie-app-instance bug.
  - >-
    PRD §13 30-day rollback window deviated. Original plan kept legacy Astro
    alive 30 days post-launch; user authorized immediate deletion 2026-05-06
    because (a) new dashboard verified working, (b) legacy instrumentation
    already broken so no rollback value, (c) discovery path preserved via
    showcase nav.
category: product
---
