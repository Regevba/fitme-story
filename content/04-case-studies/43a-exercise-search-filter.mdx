...
visual_aid:
  ...
  data:
    before:
      label: 'Before C3'
      value: '0 discoverable'
      subtitle: 'The ~50-exercise catalog carried full metadata for the entire v7.x window, but only today’s day-locked subset was visible. A user asking "what hamstring exercises does FitMe support?" had no in-app answer.'
    after:
      label: 'After C3'
      value: '~50 searchable'
      subtitle: 'A read-only library sheet on top of the existing data layer — search + 3 chip dimensions (Muscle / Equipment / Category), 4 training_ events, 7/7 tests. No schema change, no migration.'
    delta: 'invisible → searchable'
kill_criteria:
