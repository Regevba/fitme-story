...
visual_aid:
  ...
  data:
    value: '7,197'
    label: 'valid records across 8 endpoints — the locked reference store the whole sensing layer reads from (min_n=50, max_ttft 30s)'
    accentVar: 'var(--skill-qa)'
    context: 'Three producers ship on top of it — a reference store, an advisory Mahalanobis attestation, and a >3σ drift monitor — and make exactly 0 dispatch decisions. The discipline is refusing to act on a signal validated only for sensing.'
kill_criteria:
