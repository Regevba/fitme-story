...
visual_aid:
  ...
  data:
    beforeLabel: 'Before (B+C+D open)'
    before:
      label: 'Surfaces with a current, linked arch doc'
      value: '0 / 2'
      subtitle: 'iOS had no architecture doc; the web doc was stale. No maintenance protocol or drift detection.'
    afterLabel: 'After (B+C+D closed)'
    after:
      label: 'Surfaces with a current, linked arch doc'
      value: '2 / 2'
      subtitle: 'iOS (~95% mirror fidelity) + web (~90%) each have a current, no-false-claim doc, both linked from CLAUDE.md.'
    delta: '0/2 → 2/2 — kill criterion fired falsely (W38), retracted before any rebuild write'
kill_criteria:
...
honest_disclosures:
  ...
  - 'The T1 audit''s kill criterion fired falsely — `get_metadata`/`get_variable_defs` read the iOS library as an empty "Cover" page (0% fidelity) because Figma read tools reflect desktop-app context, not the `fileKey` (observed-pattern W38). It took an authoritative `use_figma` plugin-API read to contradict it before a destructive rebuild; a single-read trust would have been wrong.'
---
