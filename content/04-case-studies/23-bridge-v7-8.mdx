...
tldr: 'v7.7 shipped a gate (CACHE_HITS_EMPTY_POST_V6) that ran on every commit but exercised data on 0 of 46 features — a textbook silent-pass. The framework asserted gate *implementation*; it never asserted gate *execution*. v7.8 ships the meta-fix: Mechanism A coverage-asserting gates (every gate emits {candidates, checked, skipped, skip_reasons} per run, captured in .claude/logs/gate-coverage.jsonl), Mechanism B dual-read + canonical framework_version on 46/46 features, Mechanism C session-event auto-instrumentation with active-feature attribution wired through /pm-workflow, Mechanism D pre-commit-self-test for header drift, Mechanism E git merge driver that auto-resolves append-only ledger conflicts, and Mechanism F advisory membrane-status smartlog. M3 schema bridge fields populated on 47/47 features. M4 honesty-ledger first entry FT2-FH-001 + cold-start entrypoint published. 9 PRs across 2 days (#173 + #185–#189 + #192–#194). 27 mechanical gates + 2 advisories + the Mechanism A coverage observability layer.'
key_numbers:
...
honest_disclosures:
  ...
  - 'M3 PR-5 (schema bridge fields: agent_manifest, _meta.deprecation_warnings, path-reducers.json, agent-leases.json + migration script + fcntl.flock + epoch scaffolding) deferred — needs design judgment on field shapes.'
  - 'M3 PR-6 (Mechanism F membrane status + UCC dashboard panel + Mechanism D pre-commit header self-audit) deferred.'
  - 'M4 PR-7 (final synthesis of this case study + cold-start entrypoint + framework-honesty-ledger entry) deferred — case study is currently the live append-only journal, not yet final.'
  - 'PR #187 (Mechanism A) hit the known parallel-clone macOS-15 simulator hang in CI (`MealLogUITests.testNutritionTabOpensMealEntryPath`). The PR touched 0 Swift code; failure is environmental + matches PR #166 backlog. Re-run pending at time of merge.'
...
deferred_items:
  - title: 'M3 PR-5 — Schema bridge fields'
    ledger: 'state.json on all 46 features'
    reason: 'Largest piece — needs design judgment on field shapes (agent_manifest, _meta.deprecation_warnings, path-reducers.json, agent-leases.json).'
  - title: 'M3 PR-6 — Mechanism F membrane status + UCC dashboard panel'
    ledger: '/control-room/framework §7'
    reason: 'Needs UCC dashboard work coupled with the script. Mechanism D (pre-commit header self-audit) bundled here.'
  - title: 'M4 PR-7 — Final synthesis'
    ledger: 'this case study Section 99 + framework-honesty-ledger entry FT2-FH-001'
    reason: 'Lands when M3 PR-5 + PR-6 ship.'
...
visual_aid:
  ...
  data:
    ...
    after:
      label: 'v7.8 partial ship (M1+M2)'
      value: '27 gates + 2 advisories + coverage layer + merge driver'
      subtitle: 'Coverage observability for every gate. framework_version 46/46 canonical. Mechanism C attribution wired. Ledger conflicts auto-resolve.'
    delta: 'M1 (coverage + dual-read + framework_version backfill) + M2 (Mechanism C wiring + Mechanism E merge driver) shipped. M3 + M4 pending.'
---
