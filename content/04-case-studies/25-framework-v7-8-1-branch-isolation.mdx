---
title: >-
  Framework v7.8.1 — Branch Isolation + Feature-Closure Completeness from One
  Full PM Cycle
slug: framework-v7-8-1-branch-isolation
upstream_path: docs/case-studies/framework-v7-8-branch-isolation-case-study.md
tier: light
tldr: >-
  Two cooperating pre-commit gates shipped as one feature in advisory mode on
  2026-05-07. BRANCH_ISOLATION_VIOLATION prevents agents from mutating feature
  state from the wrong branch/worktree (Mode B fires on every commit when staged
  files match infra-path globs; Mode C fires on current_phase mutations from a
  non-feature branch). FEATURE_CLOSURE_COMPLETENESS fires on
  current_phase=complete transitions and validates 7 required case-study
  frontmatter fields + Q7 (kill_criteria_resolution) + Q6 (bidirectional PR-list
  parity). 14 commits across 9 phase transitions in one session — the first
  feature shipped via the v7.8 protocol (Mechanism C session attribution +
  isolated worktree from Phase 1 onward + Tier 2.2 logging on every phase
  transition + Mechanism A coverage telemetry verification on its own gates).
  28/28 implementation tasks done; 130/130 unit tests pass; 19/19 pipeline
  assertions pass. v7.9 promotion candidate decision: 2026-05-21 (T+14d).
timeline_position:
  version: 7.8.1
  order: 39
persona_emphasis:
...
key_numbers:
  - label: PRs merged
    ...
    tier: T1
    note: >-
      PR #244 = feature ship (squash 6d1a53f, 14 commits). #245 = Phase 8
      closure. #246 = v7.8.1 docs propagation. fitme-story #53 = paired
      dev-guide bump.
  - label: Phase transitions in one session
    value: '9'
    tier: T1
    note: >-
      Research → PRD → Tasks → Integration → Implementation → Test → Review →
      Merge → Documentation. Each transition logged via append-feature-log.py
      before state.json mutation; PHASE_TRANSITION_NO_LOG +
      PHASE_TRANSITION_NO_TIMING gates fired correctly on every commit.
  - label: Implementation tasks done
    value: 28 / 28
    tier: T1
    note: >-
      8 blocks (A schema + B branch-isolation + C closure-completeness + D
      cycle-time + E skill extensions + F make targets + G tests + H docs). T29
      (Phase 9 prioritization pass) deferred to 2026-05-21 by design.
  - label: Unit tests pass
    value: 130 / 130
    tier: T1
    note: >-
      14 new (test_branch_isolation_and_closure_completeness.py) + 116 existing.
      Mechanism D self-audit (test_pre_commit_self_test.py) clean: 16 declared =
      14 implemented + 2 inline.
  - label: Pipeline assertions
    value: 19 / 19
    tier: T1
    note: >-
      test-v7-5-pipeline.sh extended from 15 to 19 assertions (4 new for
      v7.8.1). All pass.
  - label: Files changed
    value: '25'
    tier: T1
    note: >-
      +4548 / -38. 9 extensions to existing files + 4 new files + 5 doc updates
      + 7 supporting state/log/test files.
  - label: High-risk iOS files touched
    value: '0'
    tier: T1
    note: >-
      DomainModels.swift, EncryptionService, SupabaseSyncService,
      CloudKitSyncService, SignInService, AuthManager, AIOrchestrator all
      unchanged. Pure framework feature.
  - label: New gates shipped
    value: 3 write-time + 3 cycle-time advisory
    tier: T1
    note: >-
      Write-time: BRANCH_ISOLATION_VIOLATION (advisory) +
      FEATURE_CLOSURE_COMPLETENESS (advisory) + ISOLATION_OPT_OUT_REASON_MISSING
      (enforced). Cycle-time advisory: BRANCH_ISOLATION_HISTORICAL +
      BRANCH_ISOLATION_LAUNCHD_DRIFT + FEATURE_CLOSURE_COMPLETENESS mirror.
  - label: v7.9 promotion decision date
    value: '2026-05-21'
    tier: T1
    note: >-
      T+14d post-ship. Single config-flag flip in
      framework-manifest.json::capabilities.v7_8_1_branch_isolation.gates.{name}.mode
      from 'advisory' → 'enforced', OR revert if kill criteria fire
      (false-positive rate > 20% in week 1, or pre-commit hook runtime > 10s, or
      coverage ledger shows checked=0 for 7 consecutive days).
visual_aid:
  component: HeroMetric
  data:
    value: '9 phases · 1 session'
    label: >-
      a full PM cycle — Research → Documentation — in one sitting
    context: >-
      28 / 28 implementation tasks done · 130 / 130 unit tests pass · 19 / 19
      pipeline assertions pass. The first feature shipped end-to-end via the
      v7.8 protocol (Mechanism C session attribution + isolated worktree from
      Phase 1 + Tier 2.2 logging on every transition).
kill_criteria:
  - >-
    BRANCH_ISOLATION_VIOLATION blocks > 20% of legitimate-on-main commits in
    week 1 → revert + redesign exemption logic
  - >-
    FEATURE_CLOSURE_COMPLETENESS false-positive rate > 5% on legitimate closures
    → relax field requirements
  - >-
    Auto-isolation flow fails to dispatch in > 10% of triggered cases → fall
    back to advisory-only
  - Pre-commit hook latency > 200ms p95 on the new gates → optimize or skip
kill_criterion_fired: false
honest_disclosures:
  - >-
    Both gates ship in advisory mode in v7.8 — they emit telemetry to
    gate-coverage.jsonl but do NOT block any commits. The 4 kill criteria above
    evaluate at the v7.8 → v7.9 promotion decision point (2026-05-21). v7.9
    enforcement only proceeds if all 4 thresholds are met cleanly.
  - >-
    kill_criteria_resolution is "Pending T+7d (2026-05-14) measurement window" —
    defers the actual evaluation to the post-ship window since gates fire only
    in advisory mode at v7.8.
  - >-
    First feature shipped via the v7.8 protocol on its own protocol — Mechanism
    C session attribution + isolated worktree from Phase 1 + Tier 2.2 logging on
    every phase transition + Mechanism A coverage telemetry verification on its
    own gates. The recursive verification (the gates verifying themselves) is
    the strongest signal we have today.
category: framework
---
