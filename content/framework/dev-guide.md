...
>
> **Last updated:** 2026-05-13 — **v7.8.5** ships the **observability layer**: the [Observed Patterns Catalog](https://github.com/Regevba/FitTracker2/blob/main/.claude/integrity/observed-patterns.md) (23 gate patterns + 9 workflow patterns) and the **W9 branch-drift real-time alert** ([`scripts/check-branch-drift.py`](https://github.com/Regevba/FitTracker2/blob/main/scripts/check-branch-drift.py)) — a `PostToolUse:Bash` hook that detects when a session's git HEAD has changed unexpectedly (typically caused by another concurrent Claude session sharing the same working directory running `git checkout`). Emits a LOUD stderr warning with a 4-step recovery playbook. No new write-time or cycle-time gates — purely operator-facing observability + a workflow pattern entry. Total framework mechanisms unchanged from v7.8.4: **33 mechanical gates + 5 advisories**, plus the new observability surfaces. Shipped via [FT2 PR #328](https://github.com/Regevba/FitTracker2/pull/328) (catalog) + [FT2 PR #341](https://github.com/Regevba/FitTracker2/pull/341) (W9 hook).
...
> **Filename note:** the file stays `dev-guide-v1-to-v7-7.md` for ref-stability across 16+ cross-references in FT2 + fitme-story. Content tracks the latest framework version (v7.8.5).
