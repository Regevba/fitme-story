...
> **⚠ Historical snapshot (Notion seed, not a live tracker).** Many "Medium"-band items below shipped since this was written — FIT-208/209/210/211 (chart goal lines, chart tooltips, notification settings screen, CSV export) all Done 2026-07-09; trend alerts, exercise search, and program customization also shipped. For live status use [`.claude/shared/item-registry.json`](../../.claude/shared/item-registry.json) + the Linear board.
