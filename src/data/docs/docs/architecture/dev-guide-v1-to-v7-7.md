...
>
> **Current version:** **v7.10** (shipped 2026-06-10) — observability hardening of the gates themselves: the `GATE_COVERAGE_ZERO` meta-check gained a 0-candidate mis-wire detector, three cycle-time checks (`BROKEN_PR_CITATION`, `CASE_STUDY_MISSING_TIER_TAGS`, `PATTERN_SKILL_UNMAPPED`) now emit Mechanism A coverage so the F17 index can see them, and a field-rename silent-pass class was closed in the measurement layer (observed-patterns #24). T10 AI golden-set evals also shipped. **0 new product-facing enforcement gates.** **Extended 2026-06-29** (no version bump — v7.10 chores): cross-layer naming convention (FIT-200, slug + `state.json.linear_id` + scheme-prefixed codes + `make crosswalk`), `state.json.schema_version` (DE-R18, `make migrate-state-schema`), two new advisory write-time gates `CSV_TAXONOMY_DRIFT` + `GA4_MCP_DISCONNECTED` (gate count 30→32, feature count 118→121), integrity-check parallelized (DE-R14, 9.4s→1.84s), observed-pattern W40 (cross-layer tracker lag). See CLAUDE.md "Post-v7.10 framework additions (2026-06-29)". **Extended 2026-07-03→07-04** (test-coverage batch, no version bump — 7 test/observability items, **0 new gates**, feature count 121→130, CI workflows →25): T8 WebAuthn route tests (FIT-156), T7 Playwright smoke (FIT-155), T15 orphan-test cron (FIT-163, `scripts/scan-orphan-tests.py` + `orphan-tests-weekly.yml`), R17 cross-repo state-sync health endpoint (FIT-183), + foundation slices T4 iOS snapshot (FIT-152, record-in-CI harness) and T9 backend chaos (FIT-157). See CLAUDE.md "Test-coverage batch (2026-07-03 → 07-04)". **Extended 2026-07-13→07-14** (no version bump — tooling/observability, **0 new enforcement gates**): **Option B** bot-PR CI-trigger fix — cron snapshot PRs now open via `WORKFLOW_PR_TOKEN` (a PAT) + a **signed `createCommitOnBranch`** commit + auto-merge, killing the deadlock where `GITHUB_TOKEN`-authored PRs never triggered the required checks (#884/#886; proven #887, soak verdict B19 2026-08-10; setup: [`docs/setup/bot-pr-ci-trigger-setup.md`](../setup/bot-pr-ci-trigger-setup.md)); codified **`make integrity-sweep`** — a 10-layer cross-layer data-integrity + telemetry check with one PASS/WARN/FAIL verdict (#888; **→ 11 layers as of 2026-07-21**, +"Dilution normalization" via the revived `make integrity-multi-anchor`, PR #934), now recorded daily by `daily-integrity-checkpoint.py` ([`docs/process/data-integrity-telemetry-sweep.md`](../process/data-integrity-telemetry-sweep.md)); iOS **snapshot-testing pipeline** unblocked (FIT-152/T2 #891 — 3 recorder bugs fixed incl. passing `SNAPSHOT_MODE` into the sim via the scheme's `TestAction` + an xcodebuild build-setting; 4 component baselines + advisory `ios-snapshot-verify.yml`; [`docs/process/snapshot-testing.md`](../process/snapshot-testing.md)); launchd daily-checkpoint hardened (`CRON_CONTEXT=1`, #892) after a phantom +512 empty-PR-cache regression. Counts: instrumented gates **32→33** (`SCHEMA_DIFF` T12 entered the F17 index 2026-07-10), CI workflows **25→26** (`ios-snapshot-verify.yml`), features **131** unchanged. Predecessor v7.9.1 (single-day build window, 8 observability ships across 14 PRs, closed 2026-06-04) is the most recent gate-shipping window; synthesis case study: [`framework-v7-9-1-promotion-case-study.md`](../case-studies/framework-v7-9-1-promotion-case-study.md). For v7.9 (the enforcement-flip release shipped 2026-05-21 — 3 advisory gates → enforced via single-flag flip at `scripts/check-state-schema.py:149`) see [§2.0](#20-predecessor-snapshot-v79-shipped-2026-05-21). For canonical current gate counts always defer to [`docs/FRAMEWORK-FACTS.md`](../FRAMEWORK-FACTS.md). For prior versions see [§12 timeline](#12-compressed-evolution-timeline-v10--v710).
...
| **Observed Patterns Catalog** | [`.claude/integrity/observed-patterns.md`](../../.claude/integrity/observed-patterns.md) (v7.8.5). The canonical manifest of gate-firing patterns — 25 gate patterns + 43 workflow (W1–W43). When a gate fires, consult it FIRST (§10.5). `make observed-patterns`. |
...
| **snapshot-phase / integrity-diff** | `make snapshot-phase` (v7.8.3) writes a per-phase off-SSD backup; `make integrity-diff` (v7.8.6) diffs current platform state vs the 2026-05-14 baseline anchor (raw-%, dilution-sensitive). |
| **multi-anchor / data-lake** | `make integrity-multi-anchor` (orig 2026-06-10, revived 2026-07-21 PR #934) is the **dilution-immune** companion to `integrity-diff`: it classifies every adoption-% delta vs the canonical 2026-05-14 anchor as `REAL_REGRESSION` / `dilution` / `improved` using a cohort-intersection + absolute-numerator rule, so corpus growth can't raise phantom regressions. `make integrity-data-lake` layers all telemetry into one read-only cross-source digest. Both surface as the "Dilution normalization" layer (layer 3) of `make integrity-sweep`. **2026-05-14 stays the sole canonical gating anchor** (§10.x). |
...
|------|------|--------------|----------------------|
| **A** | Coverage-asserting gates | Every write-time gate emits `{candidates, checked, skipped, skip_reasons}` per run; `GATE_COVERAGE_ZERO` advisory fires when a gate stays at `checked=0` for 7+ days. The ledger is **gitignored but shared**: `gate_coverage.canonical_ledger_path()` (#945) resolves the sink to the git **common worktree**, so a firing inside a linked worktree reaches the main checkout's ledger (the F17 index reads it) instead of a worktree-local copy discarded on `git worktree remove` — env overrides (`GATE_COVERAGE_LEDGER` / `REPO_ROOT_OVERRIDE`) preserve F16 try-repo isolation | [`.claude/logs/gate-coverage.jsonl`](../../.claude/logs/gate-coverage.jsonl) |
...
│   ├── integrity-multi-anchor.py             ← dilution-immune cohort/numerator drift (revived 2026-07-21)
│   ├── integrity-data-lake.py                ← read-only cross-source telemetry digest
...
### 10.1 Check codes (write-time + cycle-time, current through v8.x ready-now batch, 2026-06-17)

> Post-v7.10, the v8.x docket's ready-now F-items shipped: `FRAMEWORK_VERSION_STALE` (F4, #740 2026-06-16 advisory → **enforced 2026-07-08** #858), `STATE_TASKS_FILESYSTEM_DRIFT` (F1, advisory, #752 2026-06-17), `DEPENDENCY_GRAPH_CYCLE` (F3, advisory, #753 2026-06-17). The **SCHEMA_DIFF** gate (T12/FIT-160) shipped advisory 2026-07-09 (#862) as the 34th live gate — Supabase↔iOS synced-table column drift detector — and was promoted **advisory→enforced 2026-07-20** (cadence B17, #926) after a genuine field firing on 2026-07-16; see its row below. The **F16 try-repo harness was promoted advisory→enforced 2026-06-17** by adding the `try-repo-harness` CI job to main's required status checks (not a state.json gate code — an integration-test gate at the CI layer). Canonical machine-derived counts: [`docs/FRAMEWORK-FACTS.md`](../FRAMEWORK-FACTS.md).
...
| `PLATFORMS_TESTED` (T14, **advisory**; advisory→enforced 2026-06-21) | Write | `check-state-schema.py` | `current_phase=complete` transition with no platform set to `true` in `platforms_tested`. Own `PLATFORMS_TESTED_ADVISORY_MODE` flag + isolated coverage key (independent flip from FEATURE_CLOSURE_COMPLETENESS). Q2-exempt: `work_type=chore` / `work_subtype=framework_feature` / `provenance exempt:*`. Also validates field shape any phase. See §5.4. |
| `FRAMEWORK_VERSION_STALE` (F4, v8.x, **enforced 2026-07-08**, PR #858) | Write | `check-state-schema.py` | A feature advanced (a `current_phase` transition vs committed HEAD) while its recorded `framework_version` is strictly older than the canonical current version (resolved from `docs/FRAMEWORK-FACTS.md`, or `FRAMEWORK_VERSION_CANONICAL_OVERRIDE` env). Detection only — does NOT auto-mutate. Own `FRAMEWORK_VERSION_STALE_ADVISORY_MODE` flag. Exempt: reverse-sync mirrors + `framework_version_stale_exempt`. Shipped 2026-06-16 (#740). |
| `STATE_TASKS_FILESYSTEM_DRIFT` (F1, v8.x, **advisory-permanent**) | Cycle (advisory) | `integrity-check.py` | Pre-v7.6 features with empty `tasks[]` despite shipped work (cross-checks git/PR evidence, à la F2 reality-check). Shipped 2026-06-17 (#752). |
| `DEPENDENCY_GRAPH_CYCLE` (F3, v8.x, **advisory-permanent**) | Cycle (advisory) | `integrity-check.py` | Phase-2 multi-feature dependency-graph cycle/mismatch detector across roadmap features. Shipped 2026-06-17 (#753). |
| `CSV_TAXONOMY_DRIFT` (AN-1B.1, **advisory**; advisory→enforced review ~2026-07-13) | Write (advisory) | `check-state-schema.py` | Fires when `AnalyticsProvider.swift` is staged + an `AnalyticsEvent` raw value has no row in `docs/product/analytics-taxonomy.csv` (unless `csv_taxonomy_exempt`). Baseline drift 27→0 burned down 2026-06-29. Shipped 2026-06-29 (#822). |
| `GA4_MCP_DISCONNECTED` (AN-1B.2, **advisory-ONLY by design** — never blocks) | Write (advisory) | `check-state-schema.py` | Fires when analytics-affecting code is staged + GA4 MCP unreachable via env (`GA4_PROPERTY_ID` + `GOOGLE_APPLICATION_CREDENTIALS`). No calibration ladder. Shipped 2026-06-29 (#825). |
| `SCHEMA_DIFF` (T12/FIT-160, **enforced 2026-07-20**, cadence B17) | Write + cycle | `check-state-schema.py` | Supabase↔iOS synced-table column drift across `sync_records`, `cardio_assets`, `cohort_stats`. 34th live gate. Shipped advisory 2026-07-09 (#862) → promoted enforced 2026-07-20 (#926) — 1 field firing 2026-07-16, all 4 §2.2 criteria met. |
...
| **Integrity diff vs anchor** | `make integrity-diff` | Compares current platform state vs the 2026-05-14 pre-v7.9 baseline anchor (raw-%, dilution-sensitive); `EXIT_ON_REGRESSION=1` for CI | [`scripts/integrity-diff.py`](../../scripts/integrity-diff.py) |
| **Dilution normalization (multi-anchor)** | `make integrity-multi-anchor` | The **dilution-immune** companion to integrity-diff (orig 2026-06-10, revived 2026-07-21 PR #934 after the backup-path consolidation stranded it): classifies every adoption-% delta vs the canonical 2026-05-14 anchor as `REAL_REGRESSION` / `dilution` / `improved` via cohort-intersection + absolute-numerator (`classify_delta`), so corpus growth never raises phantom regressions. Only the canonical anchor gates; the 2026-06-10 anchor is trend-context only. `<2` anchors → exit 3 (loud path-drift warning). Is layer 3 of `make integrity-sweep` | [`scripts/integrity-multi-anchor.py`](../../scripts/integrity-multi-anchor.py) |
| **Telemetry data-lake** | `make integrity-data-lake` | Read-only OLAP surface layering ALL telemetry (state corpus, Mechanism A, crons, F17 index, anchors, snapshots) into one cross-source reconciliation + dilution-normalized digest; stdlib-first, optional local DuckDB; `EXIT_ON_ANOMALY=1` for CI (trips on the `R7-canonical-anchor-unresolved` HIGH finding, closing the zero-anchor silent-pass) | [`scripts/integrity-data-lake.py`](../../scripts/integrity-data-lake.py) |
...
| v8.x (ready-now batch) | 2026-06-16 → -17 | **v8.x docket ready-now F-items.** No version-bump release — these merge to main as each completes, per the cumulative v8.0 build model. **F4 `FRAMEWORK_VERSION_STALE`** (advisory, #740 → **enforced 2026-07-08** #858) — stale-version detector on phase-advance (detection, not auto-mutation; 8 emission days / 40 fires / 0 false positives). **F1 `STATE_TASKS_FILESYSTEM_DRIFT`** (advisory-permanent, #752) + **F3 `DEPENDENCY_GRAPH_CYCLE`** (advisory-permanent, #753) — cycle-time roadmap-realism checks. **F16 try-repo harness advisory→enforced 2026-06-17** (#764) — `try-repo-harness` added to main's required status checks (1d early; K2 false-positive rate 0% over 60 CI runs). With F16 enforced, the v8.0 build-kickoff gate is cleared. | All ready-now F-items are now shipped; remaining v8.x open items are date-gated (T1 → 2026-08-22, F18 post-F16-Phase-E) or operator/launch-gated (F19–F23). The F16 flip means a gate's *integration surface* can no longer regress silently into main. |
...
- [`scripts/integrity-multi-anchor.py`](../../scripts/integrity-multi-anchor.py) — dilution-immune cohort/numerator drift (revived 2026-07-21)
- [`scripts/integrity-data-lake.py`](../../scripts/integrity-data-lake.py) — read-only cross-source telemetry digest
