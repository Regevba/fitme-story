...
> Post-v7.10, the v8.x docket's ready-now F-items shipped: `FRAMEWORK_VERSION_STALE` (F4, advisory, #740 2026-06-16), `STATE_TASKS_FILESYSTEM_DRIFT` (F1, advisory, #752 2026-06-17), `DEPENDENCY_GRAPH_CYCLE` (F3, advisory, #753 2026-06-17). The **F16 try-repo harness was promoted advisory→enforced 2026-06-17** by adding the `try-repo-harness` CI job to main's required status checks (not a state.json gate code — an integration-test gate at the CI layer). Canonical machine-derived counts: [`docs/FRAMEWORK-FACTS.md`](../FRAMEWORK-FACTS.md).
