...
> Post-v7.10, the v8.x docket's ready-now F-items shipped: `FRAMEWORK_VERSION_STALE` (F4, #740 2026-06-16 advisory → **enforced 2026-07-08** #858), `STATE_TASKS_FILESYSTEM_DRIFT` (F1, advisory, #752 2026-06-17), `DEPENDENCY_GRAPH_CYCLE` (F3, advisory, #753 2026-06-17). The **SCHEMA_DIFF** gate (T12/FIT-160, advisory) shipped 2026-07-09 (#862) as the 34th live gate — Supabase↔iOS synced-table column drift detector; see its row below. The **F16 try-repo harness was promoted advisory→enforced 2026-06-17** by adding the `try-repo-harness` CI job to main's required status checks (not a state.json gate code — an integration-test gate at the CI layer). Canonical machine-derived counts: [`docs/FRAMEWORK-FACTS.md`](../FRAMEWORK-FACTS.md).
...
| `SCHEMA_DIFF` (T12/FIT-160, **advisory** during calibration) | Write + cycle (advisory) | `check-state-schema.py` | Supabase↔iOS synced-table column drift across `sync_records`, `cardio_assets`, `cohort_stats`. 34th live gate. Shipped 2026-07-09 (#862). |
