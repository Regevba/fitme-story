...
> **Status: v7.10 SHIPPED 2026-06-10** — GATE_COVERAGE_ZERO observability + field-rename closure. **Current canonical counts** (see [`docs/FRAMEWORK-FACTS.md`](../FRAMEWORK-FACTS.md)): **v7.10 · 131 features · 33 instrumented gates (21 write-time + 9 cycle-time + 2 W9 hooks + 1 standalone), 28 firing · 0 integrity findings** (reconciled 2026-07-14). The older "37 mechanical gates + 5 advisories" framing below counted mechanisms+CI-workflows+hooks; the 33-gate figure counts gate codes in the F17 index — both are real, they count different things. Calibration ladder: F16 enforced 06-17 · PLATFORMS_TESTED enforced 06-21 · W9 HELD advisory (event-gated) · R9→GATE_TEST_MISSING shipped 07-04 · CSV_TAXONOMY_DRIFT enforced 07-13 · Data Freshness Audit 08-12.
>
> **Status: v7.9.1 BUILD WINDOW CLOSED 2026-06-04 — 8 ships, 14 PRs.** v7.9.1 opened at v7.9 Phase E exit (2026-06-04) and closed the same day. **0 new enforcement gates** added (Phase E exit discipline preserved). Ships: F16 try-repo harness + F17 last_fired_at index + F2 Phase 0 reality-check sub-step + Dev-env Track B (R7/R8/R12 lint trio) + F-LAUNCHD-DRIFT-EXTENSION (b)+(c)+(a) + observed-patterns W29-W32 catalog batch + F-PHASE-E-ADOPTION-FREEZE-DISCIPLINE + R9 Track B coverage aggregator + dev-env R11/R13/R14/R17/R18 hygiene batch + F-DEPLOYED-URL-PROBE FT2 substrate. **Quantitative roll-up:** CI workflows 8 → 14 baseline (+6 warn-only: lint, coverage, gitleaks, pip-audit, sbom, commitlint, shellcheck); observed-patterns W1-W28 → W1-W32; v7.9.1 docket 7 candidates → 2 open (both fitme-story-side). **Synthesis case study:** [`framework-v7-9-1-promotion-case-study.md`](../case-studies/framework-v7-9-1-promotion-case-study.md). **Calendar:** T+7d F-LAUNCHD + F-DEPLOYED-URL-PROBE verification 2026-06-11; External Audit #2 2026-06-12; F16 T11 advisory→enforced flip 2026-06-18.
>
> **Status: v7.9 PROMOTION (predecessor) SHIPPED 2026-05-21** via PR #417 `ea53ff4` — 3 advisory gates → enforced through single-line `BRANCH_ISOLATION_ADVISORY_MODE = True → False` flip at [`scripts/check-state-schema.py:132`](../../scripts/check-state-schema.py). Gates promoted: `BRANCH_ISOLATION_VIOLATION` Mode B + Mode C + `FEATURE_CLOSURE_COMPLETENESS`. All 4 promotion criteria met against 14d Mechanism A telemetry. **Total framework mechanisms post-promotion: 37 mechanical gates + 5 advisories.** Phase E validation soak 2026-05-21 → 2026-06-04 PASSED at the 2026-05-28 B2 baseline (0 rollbacks, 0 false positives across 44 gate firings). See [CLAUDE.md "v7.9 Promotion Release" section](../../CLAUDE.md) + [cold-start entrypoint](../../.claude/entrypoints/framework-v7-9.md) + [v7.9 promotion case study](../case-studies/framework-v7-9-promotion-case-study.md).
