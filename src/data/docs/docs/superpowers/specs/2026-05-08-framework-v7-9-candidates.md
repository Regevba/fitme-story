...
| ID | Gap observed | Proposed v7.9 mechanism | Class |
|---|---|---|---|
| ~~**F14**~~ ✅ | ~~4 write-time gates have thorough unit tests for the internal check function but NO test that exercises the `main()` dispatch path~~ **SHIPPED 2026-05-23** via FT2 PR #451 squash `86084c4` + backfill #452 + closure #455. | Per-gate `test_main_dispatch_<gate>()` requirement implemented: 9 dispatch tests added to `scripts/tests/test_check_state_schema.py` + new `scripts/tests/test_integrity_check_dispatch.py` + new `scripts/tests/test_ensure_pr_cache_fresh.py`. Combined coverage 1/19 → 10/19 = 53% (write-time 1/16 → 8/16; cycle-time 0/3 → 2/3 = 67%). 161/161 pytest pass. T1 `GATE_TEST_MISSING` meta-gate (RICE 53.3) opened in backlog as successor enforcement step. | Test discipline (write-time) |
| ~~**F15**~~ ✅ | ~~5 gates have ZERO unit test coverage~~ **SHIPPED 2026-05-23 via same PR #451** (joint scope with F14). | All 5 covered: `PHASE_TRANSITION_NO_LOG`, `PHASE_TRANSITION_NO_TIMING`, `BRANCH_ISOLATION_HISTORICAL`, `BRANCH_ISOLATION_LAUNCHD_DRIFT`, `PR_CACHE_STALE`. | Test discipline (Class B closure) |
