...
| R9 | Add coverage instrumentation (Slather + c8 + coverage.py) | **[~]** TRACK A SHIPPED 2026-05-25 | iOS `.slather.yml` (FT2) + ai-engine `[tool.coverage.*]` in `pyproject.toml`. Web `c8` ships via companion fitme-story PR. Track B (Makefile `coverage-report` aggregator + CI workflow) is infra-glob, isolated worktree, post-Phase-E. |
