...
export const GLOSSARY: GlossaryEntry[] = [
  ...
  },
  {
    ...
    full: 'The `/control-room/*` routes on fitme-story.vercel.app expose the framework live state to anyone with the URL: `/control-room/framework` (framework health, all 28 gates, trend charts), the dispatch replay, and per-feature status. Originally an Astro dashboard inside FitTracker2; ported to Next.js as part of the Unified Control Center work.',
  },
  {
  ...
  },
  {
    ...
    full: 'The v8.x build window opened 2026-06-17 once the kickoff gate cleared, building on v7.10 (2026-06-10). Its headline change: F16, the try-repo pre-commit harness (the 3rd layer of gate testing that spawns a throwaway git repo and runs the real pre-commit hook via subprocess), was promoted advisory→enforced on 2026-06-17 — one day early. The mechanism is NOT a code flag: the `try-repo-harness` CI job was added to main’s required status checks via `gh api` branch protection, making it an integration-test gate at the CI layer. Calibration: K2 false-positive rate 0% over 60 CI runs / 13 days. A gate’s integration surface can no longer regress silently into main. Alongside F16, three v8.x docket "ready-now" gates shipped: F4 `FRAMEWORK_VERSION_STALE` (write-time advisory, PR #740, 2026-06-16 — fires when a feature is advanced while its framework_version is older than canonical; detection only, NOT auto-mutation; advisory→enforced ~2026-06-30; the 18th write-time gate), F1 `STATE_TASKS_FILESYSTEM_DRIFT` (cycle-time advisory-permanent, PR #752, 2026-06-17), and F3 `DEPENDENCY_GRAPH_CYCLE` (cycle-time advisory-permanent, PR #753, 2026-06-17). Current canonical counts per FT2 docs/FRAMEWORK-FACTS.md: 115 features tracked, 28 instrumented gates (17 write-time emitting + 9 cycle-time + 2 W9 hooks; F4 is an 18th write-time advisory gate). T14 PLATFORMS_TESTED was promoted advisory→enforced 2026-06-21 (PR #781). Extended 2026-06-29 (v7.10 chores, no version bump): gate count reached 32 (20 write-time + 9 cycle-time + 2 W9 + 1 standalone), 121 features. See FT2 docs/FRAMEWORK-FACTS.md for the live current-state reference.',
  },
  {
  ...
  },
  {
    ...
    full: "Shipped 2026-06-29 (DE-R18). Every .claude/features/*/state.json carries an integer schema_version (current = 1; all 121 features backfilled). scripts/migrate-state-schema.py (`make migrate-state-schema`) holds an ordered registry of (from, to, transform) migration steps and applies every pending step to bring a file to CURRENT_SCHEMA_VERSION — idempotent, dry-run by default. This closes the field-rename incident class (the historic created/created_at mismatch where a reader silently saw half the data): future schema changes append a migration step and bump the version instead of renaming fields in place.",
  },
  {
