...
export const SKILLS: Skill[] = [
  ...
  },
  {
    slug: 'design',
    displayName: '/design',
    oneLiner: 'How it Looks. Tokens, auto Figma builds, WCAG, preflight + pre-merge gates (v4.X).',
    accent: '#EC4899',
    accentVar: 'var(--skill-design)',
    ring: 'inner',
    phaseOwnership: ['P3', 'P6'],
    subCommands: ['audit', 'tokens', 'accessibility', 'preflight', 'pre-merge-review', 'prompt', 'build'],
    purpose: '/design owns the visual layer: token governance, Figma MCP builds (auto-dispatched at Phase 3.j as of v4.X 2026-05-06), component library maintenance, WCAG AA checks. v4.X adds three mechanical gates: /design preflight (P0 gate at Phase 3.f — DS compliance + Figma MCP liveness + library accessibility check), /design build auto-dispatch with Figma node ID write-back to state.json, and /design pre-merge-review (P0 gate at Phase 6.c — make ui-audit P0=0 + figma_node_ids populated + PR description references those node IDs).',
    invokes: ['ux'],
    ...
    standaloneExample: 'Used alone: /design build {feature} pushes the feature\'s screens into Figma via MCP and writes node IDs back to state.json.figma_node_ids. Falls back to a portable prompt when MCP is unreachable.',
