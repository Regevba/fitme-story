...
export const SKILLS: Skill[] = [
  ...
  },
  {
    ...
    oneLiner: 'Problem framing, assumption surfacing & UX/Design/Dev trade-off matrices for new features.',
    accent: '#14B8A6',
    accentVar: 'var(--skill-brainstorm-pm)',
    ring: 'inner',
    phaseOwnership: ['P0'],
    subCommands: ['problem', 'solution', 'assumption', 'strategy', 'three-option'],
    purpose: '/brainstorm-pm expands the option space before the user commits — surfaces problem framings, candidate solutions, hidden assumptions, strategic angles, and (since 2026-06-03) UX/Design/Dev trade-off matrices for scoped problems with multiple viable implementation paths. Default Phase 0 entry point for /pm-workflow whenever the problem shape is not obvious. Distinct from superpowers:brainstorming which is generic creative work; every /brainstorm-pm output is reducible to a PRD section (problem statement, success metric, kill criterion, JTBD, opportunity branch, or alternatives-considered matrix). The three-option mode produces a ≥3-row matrix across UX/Design/Dev/defer-to-v2/failure-modes/effort dimensions; NO pre-ranking — user picks.',
    invokes: ['research'],
    ...
    standaloneExample: 'Used alone: /brainstorm-pm problem on "we should add a workout AI coach" reframes the user-stated want into a recommendation problem before AI scope is locked. Used with three-option mode on "stats expansion for richer recovery insights": produces 3 options on the compute axis (in-app summarization / cloud LLM / hybrid prefetch), each with UX, Design, Dev rows + deferred-to-v2 column + failure modes; mid-tier option drops into PRD §Alternatives considered.',
  ...
  },
  {
    slug: 'ux',
    displayName: '/ux',
    oneLiner: 'What & Why. Research, specs, audits, preflight + pre-merge gates (v4.X).',
    accent: '#D946EF',
    accentVar: 'var(--skill-ux)',
    ring: 'inner',
    phaseOwnership: ['P3', 'P6'],
    subCommands: ['research', 'spec', 'wireframe', 'validate', 'preflight', 'pre-merge-review', 'audit', 'patterns', 'prompt'],
    purpose: '/ux owns UX planning — principles, specs, wireframes, v2 audits, validation. Split from /design in v2.0 to keep "what & why" separate from "how it looks." v4.X (2026-05-06) adds two mechanical gates: /ux preflight (P0 gate at Phase 3.e — verifies every token/component/pattern named in ux-spec.md exists in the codebase before Phase 4 starts) and /ux pre-merge-review (P0 gate at Phase 6.b — heuristic re-check of shipped code vs approved spec before merge).',
    invokes: ['research', 'design'],
    invokedBy: ['pm-workflow', 'design'],
    readsFromShared: ['feature-registry.json', 'context.json'],
    writesToShared: ['feature-registry.json', 'ux-spec-preflight.json'],
    standaloneExample: 'Used alone: /ux preflight import-training-plan verifies every token + component the spec invokes actually exists. Catches "AppRadius.pill" doesn\'t exist before Phase 4 hits "no such symbol."',
    docsHref: `${GH_DOCS}/ux.md`,
  },
  {
    slug: 'design',
    displayName: '/design',
    oneLiner: 'How it Looks. Tokens, auto Figma builds, Code Connect bridge (web + iOS), WCAG, preflight + pre-merge gates (v4.X).',
    accent: '#EC4899',
    accentVar: 'var(--skill-design)',
    ring: 'inner',
    phaseOwnership: ['P3', 'P6'],
    subCommands: ['audit', 'tokens', 'accessibility', 'preflight', 'pre-merge-review', 'prompt', 'build'],
    purpose: '/design owns the visual layer: token governance, Figma MCP builds (auto-dispatched at Phase 3.j as of v4.X 2026-05-06), component library maintenance, WCAG AA checks, and the cross-repo Figma↔code bridge via Code Connect (added 2026-05-09 — .figma.tsx mappings for the React side at fsjHfFLAHELACZHku8Rfcl + .figma.swift mappings for the SwiftUI side at 0Ai7s3fCFqR5JXDW8JvgmD). v4.X adds three mechanical gates: /design preflight (P0 gate at Phase 3.f — DS compliance + Figma MCP liveness + library accessibility check), /design build auto-dispatch with Figma node ID write-back to state.json, and /design pre-merge-review (P0 gate at Phase 6.c — make ui-audit P0=0 + figma_node_ids populated + PR description references those node IDs). Companion docs: docs/design-system/ios-code-connect-workflow.md (operator runbook) + docs/design-system/fitme-story-design-architecture.md (web architecture).',
    invokes: ['ux'],
    invokedBy: ['pm-workflow', 'ux'],
    readsFromShared: ['design-system.json', 'feature-registry.json', 'figma-bridge-status.json'],
    writesToShared: ['design-system.json', 'figma-bridge-status.json', 'figma-code-sync-status.md'],
    standaloneExample: 'Used alone: /design build {feature} pushes the feature\'s screens into Figma via MCP and writes node IDs back to state.json.figma_node_ids. Falls back to a portable prompt when MCP is unreachable. Code Connect mappings (.figma.tsx + .figma.swift) auto-scaffold via scripts/scaffold-figma-mapping.{py,mjs} once node IDs are captured.',
  ...
  },
  {
    ...
    ring: 'outer',
    phaseOwnership: ['P0', 'P9'],
    subCommands: ['reviews', 'nps', 'sentiment', 'testimonials', 'roadmap', 'digest', 'analyze'],
    purpose: '/cx ingests app-store reviews, NPS scores, sentiment signals, and post-deployment feedback. As an outer-ring skill, it observes the shipped product after Release (P8) and feeds back to Research (P0) and Learn (P9) — closing the lifecycle loop.',
  ...
  },
  {
    ...
    purpose: '/marketing owns outbound communication: App Store optimization, campaigns, competitive positioning, launch coordination. Outer-ring: it observes the shipped product after Release (P8) — funnel data, ASO performance, campaign response — and feeds market intelligence back to Research (P0) for next-cycle prioritization.',
    invokes: [],
  ...
  },
  {
    ...
    ring: 'inner',
    phaseOwnership: ['P7', 'P8'],
    subCommands: ['prepare', 'checklist', 'notes', 'submit'],
    purpose: '/release owns the pre-ship gauntlet AND the public ship moment: version bumps, changelog generation, TestFlight preparation, App Store submission, and the actual go-live at P8 Release. Phase 7 (Merge) is the integration step; Phase 8 (Release) is the moment the feature is live for users and the three feedback skills (CX, Ops, Marketing) start observing.',
    invokes: ['dev', 'qa'],
    invokedBy: ['pm-workflow'],
